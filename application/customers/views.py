...
def get_all_pending_bookings(cust_id):
    ...
    bookings = (
        ...
        .filter(Booking.status.in_([BookingStatusEnum.PENDING.value, BookingStatusEnum.CONFIRM.value]), Customer.id==cust_id)
        .all()
    )
...
def get_all_payments(cust_id):
    ...
    payments = (
        ...
        .join(Customer, Booking.customer)
        .filter(Customer.id==cust_id, CustomerPayment.status.notin_([CustomerPaymentStatusEnum.PENDING.value]))
